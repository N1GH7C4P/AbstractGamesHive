...
function Input.keypressed(key)
    ...
    if key == "a" and G.active_piece_id < maxPieceId then
        G.active_piece_id = G.active_piece_id + 1
    elseif key == "s" and G.active_piece_id > 1 then
        G.active_piece_id = G.active_piece_id - 1
    elseif key == "c" then
        console.toggle()
    elseif key == "x" then
...
function Input.mousepressed(x, y, button, istouch)
    ...
    if button == 1 then
        ...
        if G.mosquito_choice_popup then
        ...
        if G.move_mode == 1 then
            ...
            if G.pillbug_special_mode and G.pillbug_target_cube then
